...
def write_evolution(primary, secondary, current_time, L_x, P, P_dot, M_dot, M_loss, B, L_gamma, w_s, AIC):
...
def print_evolution(primary, secondary, current_time):
    ...
    print("Current time:", current_time.as_quantity_in(units.Myr))
...
def main_evolution(secondary, primary, current_time, B, printing, recycling, dt, MSP):
    ...
    while primary.mass.value_in(units.MSun)>0 and current_time< 15000 |units.Myr:
        if 10<= primary.stellar_type.value_in(units.stellar_type) <=12:
        ...
        if MSP == True:
            if primary.stellar_type.value_in(units.stellar_type) != 13:
                ...
                dt = dt_orig
            # if primary_old_type == 12 and primary.stellar_type.value_in(units.stellar_type) == 13:
        ...
        if primary.stellar_type.value_in(units.stellar_type) == 12 and primary_old_type == 12:     ##WD
            ...
            # elif M_dot > 0:       ## Does not really work,neither does it matter because of weak magnetic fields
        ...
        if P_new <= P_breakup and secondary.mass.value_in(units.MSun) >= M_secondary_limit:           ## NS gains mass (might not be realistic) and shrinks. 
            if recycling == True:                                                               ## Thus we have a new breakup period. Essentially a rotationally supported NS
                ...
                # P_ = ( 2 * np.pi /w ) * 3.154e+7    #seconds
                # if P_ <= P_breakup:
                #     primary.spin = w_breakup |units.none
                # else:
                #     primary.spin = w |units.none
        ...
        if primary.stellar_type.value_in(units.stellar_type) == 13:
            write_evolution(primary, secondary, current_time, L_x, P, P_dot, M_dot, M_loss, B, L_gamma, w_s, AIC=True)
        elif primary.stellar_type.value_in(units.stellar_type) == 13 and primary_old_type == 12:
            write_evolution(primary, secondary, current_time, L_x, P, P_dot, M_dot, M_loss, B, L_gamma, w_s, AIC=2) 
        else:
            write_evolution(primary, secondary, current_time, L_x, P, P_dot, M_dot, M_loss, B, L_gamma, w_s, AIC=False) 
        ...
        if printing == True:
            ...
            print_evolution(primary, secondary, current_time)
    ...
    return secondary, primary, current_time, primary_old, secondary_old, a_old
...
def evolve_binary(M_wd_zams, M_secondary_zams, a_zams, B):
    secondary = stars[1]
    primary = stars[0]
    ...
    secondary, primary, current_time, primary_old, secondary_old, a_old = main_evolution(secondary, primary, current_time, B, printing, recycling, dt, False)
    ...
    current_time = round((current_time - dt).value_in(units.Myr)/10)*10 |units.Myr
    # dt=1e3|units.yr
    if AIC == True:
        MSP = True
        ...
        secondary, primary, current_time, primary_old, secondary_old, a_old = main_evolution(stars[1], stars[0], current_time, B, printing, recycling, dt, MSP)
...
def parallel_evolution(sliced_data, slice_length, x, B_slice, outdir):
        ...
        for i in range(0, slice_length):
            ...
            AIC = evolve_binary(M_wd_zams[i], M_secondary_zams[i], a_zams[i], B_slice[i] )
...
if __name__ == "__main__":
    ...
    # data = read_data("AIC_final_imp.dat", data)
    ...
    if ncores == 1:
        parallel_evolution(data_slice[0], slice_len[0], 0, B_slice[0], outdir)
    else:
        futures = []
        for x in range(0, ncores):
            futures.append( executor.submit( parallel_evolution, data_slice[x], slice_len[x], x, B_slice[x], outdir) )
