...
def separate_data():
    ...
    for i in range(0, length):
        ...
        Mdot_accr.append( np.abs( min( data[i,12], data[i,13] ) ) |units.MSun/units.yr )
        Mdot_loss.append( - np.abs( max( data[i,12], data[i,13] ) ) |units.MSun/units.yr )
        
        t_start.append( data[i,0] * 1e6 |units.yr )
        t_end.append( data[i,6] * 1e6 |units.yr)
        a_init.append( data[i,5] |units.RSun)      
...
def initial_donor_evolution(M_donor, donor_type):
    stev = MESA(redirection='none')
    stev.parameters.AGB_wind_scheme = 0
    stev.parameters.RGB_wind_scheme = 0
    # dt = 1e5
    star = stev.particles.add_particle(Particle( mass = M_donor ))
    old_stellar_type = star.stellar_type
    while star.stellar_type <= (donor_type | units.stellar_type):
        # if star.stellar_type != old_stellar_type: 
        #     dt = dt/10
        # star.time_step = dt|units.yr
        star.evolve_one_step()
        old_stellar_type = star.stellar_type
        print('Time step :', star.time_step, '->', "Evolved to:", star.age, "->", star.radius)
...
def time_steps(nM, M_donor, star_donor, wd_type, donor_type, Mdot_accr, nu, Mdot_loss, t_start, a, M_limit, k, t1):
    ...
    while nM[k] < M_limit and M_donor[k] >= 0.1|units.MSun and t <= 13.7e9|units.yr:
        if star_donor.radius > roche_radius(nM[k], M2, a[k]):
            star_donor.mass_change = Mdot_loss
            star_donor.time_step = dt|units.yr
            star_donor.evolve_one_step()
            M_donor.append( star_donor.mass )
            ...
            star_donor.time_step = 1e7|units.yr
            star_donor.evolve_one_step()
            M_donor.append( M_donor[k] )
            nM.append( nM[k] )
            ...
            M1 = nM[k] + M_disk
            M2 = M_donor[k]
            a.append( orbit(M1, M2, a[k], 0|units.MSun/units.yr, 0|units.MSun/units.yr, 1e7|units.yr)) 
            t += 1e7|units.yr
            t1.append( t1[k] + 1e7|units.yr )
...
def time_evolution(M, M_don, Mdot_accr, Mdot_loss, wd_type, donor_type, t_start, a_init):
    ...
    star_donor = initial_donor_evolution(M_don, donor_type)
    # star_donor.parameters.AGB_wind_scheme = 0
    # star_donor.parameters.RGB_wind_scheme = 0
    ...
    if wd_type == 12: 
        ...
        if aic == True:               # NS Recycling 
            ...
            nM, M_donor, star_donor, a, k, t, t1, M1, merger= time_steps(nM, M_donor, star_donor, wd_type, donor_type, 2.92316e-8 |units.MSun/units.yr, 0.8, Mdot_loss, t_aic, a, 3|units.MSun, k, t1)
...
if __name__ == '__main__':
    ...
    M_ch = 1.38 |units.MSun
    c = 9.4605284e12  |units.km /units.yr
    R_solar = 695700 |units.km
