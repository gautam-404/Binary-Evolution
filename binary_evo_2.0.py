...
def time_steps(nM, M_donor, donor_type, Mdot_accr, nu, Mdot_loss, t_start, a, M_limit, k, t1):
    M_disk = 0
    ...
    M1 = nM + M_disk
    M2 = M_donor 
    a_min = ( donor_radius(M2, donor_type) + (wd_mass_to_radius(nM)/R_solar) )
    while nM < M_limit and a[k]> a_min and M_donor >= 0.1 and t <= 13.7e9:
        if donor_radius(M2, donor_type) > roche_radius(M1, M2, a[k]):
        ...
        elif donor_radius(M2, donor_type) < roche_radius(M1, M2, a[k]) and M_disk > 0:
...
def time_evolution(M, M_donor, Mdot_accr, Mdot_loss, wd_type, donor_type, t_start, a_init):
    ...
    if wd_type == 12: 
        nM, M_donor, a, k, t, t1, M1 = time_steps(nM, M_donor, donor_type, Mdot_accr, 0.4, Mdot_loss, t_start, a, M_ch, k, t1)
        if nM>=M_ch:                        # AIC
            aic = True 
            t_aic = t
            k_aic = k
        ...
        # if aic == True:               # NS Recycling 
            ...
            nM = M_ns
            nM, M_donor, a, k, t, t1, M1 = time_steps(nM, M_donor, donor_type, 2.92316e-8, 0.8, Mdot_loss, t_aic, a, 3, k, t1)
            w_rns = spin_up(nM, M_ns, r, w_ns)
            P_rns = ( 2 * np.pi /(w_rns) ) * 3.154e+7    #seconds
            print(a[k])
            print("\n")
            plt.plot(np.log10(t1[0:k_aic]), np.log10(a[0:k_aic]), color = 'orange')
